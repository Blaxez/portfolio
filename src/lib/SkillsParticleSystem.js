...
export class SkillsParticleSystem {
  ...
  async getPointsFromImage(source) {
    ...
    if (source.startsWith("http") || source.startsWith("/")) {
      ...
      url = `https://cdn.simpleicons.org/${source}/white`;
    }
  ...
  async loadSkill(index) {
    ...
    // Sync secondary color to ensure the particle looks exactly like the skill color
    // We can make it slightly brighter or keep it identical for purity.
    // User requested "exactly same", so we will use the same base hue.
    this.material.uniforms.uColor2.value.set(skill.color);
  }
